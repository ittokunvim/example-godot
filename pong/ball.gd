...
const REFLECTION_ANGLE_FACTOR := 1.25
...
func _process(delta: float) -> void:
	if _direction != Vector2.ZERO:
		...
		position.y = clampf(position.y, BALL_RADIUS, _screen_size.y - BALL_RADIUS)
...
func _on_area_entered(area: Area2D) -> void:
	if area.is_in_group("walls"):
		...
		_reflect_from_paddle(area)
	$BounceSound.play()


func _reflect_from_paddle(paddle: Area2D) -> void:
	var paddle_visual := paddle.get_node("ColorRect") as ColorRect
	var paddle_center: float = paddle.position.y + paddle_visual.size.y / 2.0
	# パドルの中心からどれだけ離れて当たったかを、-1から1に正規化する。
	var hit_position: float = clampf(
		(position.y - paddle_center) / (paddle_visual.size.y / 2.0),
		-1.0,
		1.0
	)
